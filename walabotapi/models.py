...
class Fallback(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    payload_string = db.Column(db.String(2048), unique=False, nullable=False)
    origin = db.Column(db.String(8), unique=False, nullable=False)
    
    def __repr__(self):
        return f"Fallback('{self.payload_string}', from '{self.origin}')"
